...
- **도전과제:** `challengeDefs` — 판매 등록가 1건 ≥ N / 아이템 획득 N회(띄어쓰기 무시·부분 일치) · 보상 칭호+XP. **칭호 아이콘**은 관리 UI 없음 → 추가 시 코드/`challengeDefs.icon` 또는 요청 시 에이전트가 `image/훈장아이콘/` 연결.
...
*Last updated: 2026-09-18 (공대원 카드 UI)*
